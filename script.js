...
const requestMediaPermissions = async () => {
    try {
        ...
        alert('Camera and microphone access denied. You can still use text chat.');
    }
};
...
if (customId) {
    username = prompt('Enter your username (e.g., Alice):');
    if (username) {
        initializePeer(customId); // Initialize PeerJS with the custom ID
        requestMediaPermissions(); // Request camera and microphone access
    } else {
...
callButton.addEventListener('click', () => {
    ...
    if (!localStream) return alert('Camera and microphone access denied. Cannot start video call.');
